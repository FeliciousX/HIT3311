...
=========
## Decision Framing ##
How we frame the problem can change the decision we make.
Framing an option as a `loss` increases the chance that you will gamble.
Framing an option as a `sure gain` increases the chance that you will not take risk.

## Software is not normal ##
Software metric data always shows a highly skewed distribution and not like a bell curve.
This indicate that there is a bias by developers in a few factors.

### Gini Coefficient ###
To summarize a skewed distribution, we can use the Gini Coefficient.
It captures the inequality within a distribution.
Most software has a Gini Coefficient between 0.45 - 0.75

Higher Gini = Higher concentration of Functionality and complexity in a few modules
Lower Gini = Dispersion of Functionality in many modules

Dispersion increases abstraction. More thing to remember for developers when developing.
